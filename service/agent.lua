...
function cli:login()
	...
	if data.fd then
		LOG_INFO("login fail %s fd=%d", data.userid, self.fd)
		return { ok = false }
	end
	data.fd = self.fd
	self.login = true
	LOG_INFO("login succ %s fd=%d", data.userid, self.fd)
...
local function new_user(fd)
	...
	LOG_INFO("fd=%d is gone. error = %s", fd, error)
	--client.close(fd)
	if data.fd == fd then
		data.fd = nil
		...
		if data.fd == nil then
			...
			if not data.exit then
				...
				LOG_INFO("user %s afk", data.userid)
				skynet.exit()
			end
		end
	end
